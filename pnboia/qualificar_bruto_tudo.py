...
def consulta_banco(boia):
    ...
    argosbruto=[]
    cur.execute("SELECT * FROm pnboia_bruto wheRe estacao_id=%s\
    ORdeR by data" %boia)
...
def boiasfuncionando():
    ...
    boias=[]
    cur.execute("SELECT * FROM pnboia_estacao")
    for row in cur.fetchall():
        boias.append(row)
    return boias
    cur.close()
...
for s in range(len(boias)):
    ...
    argosbruto=consulta_banco(boias[s][0])
    ...
    data1=[]
    data1=[data,Wdir1,Wspd1,Gust1,Wdir2,Wspd2,Gust2,Wvht,Wmax,Dpd,Mwd,Pres,Humi,Atmp,Wtmp,Dewp,Cvel1,Cdir1,Cvel2,Cdir2,Cvel3,Cdir3,Battery]


    (Wspd,Wdir,Gust,flag,flagid)=qualitycontrol(Epoca,data,Wdir1,Wspd1,Gust1,Wdir2,Wspd2,Gust2,Wvht,Wmax,Dpd,Mwd,Pres,Humi,Atmp,Wtmp,Dewp,Cvel1,Cdir1,Cvel2,Cdir2,Cvel3,Cdir3,Battery,Lat,boias[s][2])
