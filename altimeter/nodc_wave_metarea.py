...
for i6 in range(len(file_list)):
    for f in  file_list[i6]:
        print(f)
        NC = Dataset(f,'r')
        #DEFININDO AS VARIAVEIS NO NETCDF
        for i3 in range(len(variables)):
            if i3==0:
                tempo=NC.variables['time']
            else:
                exec("%s=NC.variables['%s']"% (variables[i3],variables[i3]))
        ...
        # A ROTINA SYS EH SOMENTE PARA CONTROLE DO ANDAMENTO DAS BUSCAS. PODE SER RETIRADA
        #        sys.stdout.write("\r%s" %(sta[ii][0]))
        #        sys.stdout.flush()
...
if dia1!=dia:
    ...
    while hour<=hora:
        if hour<10:
            ...
            valor=str(hour)
        glob_pattern = "*JA2_OPR_*_*_"+str(ano)+mes+dia+"_"+valor+"*_*_*.nc"
        if glob.glob(glob_pattern)!=[]:
            file_list.append(glob.glob(glob_pattern))
        hour=hour+1
    while hora1<=23:
        if hora1<10:
            ...
            valor=str(hora1)
        glob_pattern = "*JA2_OPR_*_*_"+str(ano)+mes1+dia1+"_"+valor+"*_*_*.nc"
        if glob.glob(glob_pattern)!=[]:
            file_list.append(glob.glob(glob_pattern))
        hora1=hora1+1
else:
...
origem=[]
for i in range(len(day_med)):
    origem.append('jason2')
    datahora_med.append(datetime.datetime.strptime(str(day_med[i])+"/"+str(month_med[i])+"/"+str(year_med[i])+" "+str(hour_med[i])+":"+str(minute_med[i])+":00", '%d/%m/%Y %H:%M:%S'))

Data1=np.array([origem,datahora_med,lat_med,lon_med,swh_med])
Data1 = Data1.transpose()
# header="year,month,day,hour,minute,lat,lon,swh"
# np.savetxt("jason02.csv", Data1,'%s',delimiter=" ",header=header)

# import xlsxwriter
# import math
# workbook=xlsxwriter.Workbook("jason02.xls")
# worksheet=workbook.add_worksheet()
...
#         worksheet.write(i+1,ii,Data1[i][ii])
